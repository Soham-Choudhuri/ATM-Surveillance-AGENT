  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 flex flex-col font-sans ${isAlarming ? 'bg-rose-600 animate-[pulse_1s_ease-in-out_infinite]' : 'bg-neutral-950'} text-white`}>
      ...
      <main className="flex-1 p-4 md:p-8 flex flex-col items-center justify-center">
        {isAlarming ? (
          <div className="bg-black/40 backdrop-blur-2xl border border-white/20 p-6 md:p-12 rounded-2xl md:rounded-3xl max-w-4xl w-full text-center shadow-2xl">
            <svg className="w-20 h-20 md:w-32 md:h-32 text-white mx-auto mb-4 md:mb-6 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
            <h2 className="text-3xl md:text-6xl font-black mb-2 md:mb-4 tracking-tight drop-shadow-lg">CRITICAL INCIDENT DETECTED</h2>
            <div className="bg-rose-900/50 p-4 md:p-6 rounded-xl border border-rose-500/30 mb-6 md:mb-8">
                <p className="text-lg md:text-2xl font-medium mb-1 md:mb-2">{state?.latest_report?.classification}</p>
                <p className="text-base md:text-xl text-rose-100">{state?.latest_report?.description}</p>
            </div>
            <button 
              onClick={() => setAcknowledgedId(currentIncidentKey)}
              className="w-full md:w-auto bg-white text-rose-600 hover:bg-neutral-200 px-6 py-4 md:px-12 md:py-6 rounded-xl md:rounded-2xl text-lg md:text-2xl font-black uppercase tracking-widest transition-transform hover:scale-105 active:scale-95 shadow-xl"
            >
          ...
          <div className="text-center opacity-50 px-4">
             ...
             {state?.latest_report && (
                 <div className="mt-8 md:mt-12 bg-white/5 border border-white/10 p-4 md:p-6 rounded-xl md:rounded-2xl max-w-2xl mx-auto text-left">
                     ...
                     <p className="text-sm md:text-base text-neutral-300">{state.latest_report.description}</p>
                 </div>
             )}
          </div>
        )}
